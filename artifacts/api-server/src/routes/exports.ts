...
router.get("/exports/transactions.csv", requireAuthOrQueryToken, async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  res.setHeader("Content-Disposition", `attachment; filename="iapay-transactions-${new Date().toISOString().slice(0, 10)}.csv"`);
  res.send("\uFEFF" + csv);
});
...
router.get("/exports/transactions.json", requireAuthOrQueryToken, async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  res.setHeader("Content-Disposition", `attachment; filename="iapay-transactions-${new Date().toISOString().slice(0, 10)}.json"`);
...
<div class="logo">IAPAY</div><div class="sub">INTER-AFRICA PAY &middot; OFFICIAL RECEIPT</div>
