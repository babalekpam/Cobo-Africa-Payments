// IAPAY settlement scheduler — real schemes settle on a clock, not on demand.
...
export function startSettlementScheduler(): void {
  ...
  if (!hours || hours <= 0) {
    logger.info("IAPAY settlement scheduler disabled (SETTLEMENT_INTERVAL_HOURS <= 0)");
    return;
  }
    ...
    try {
      ...
      if (summary) {
        logger.info(
          ...
          "IAPAY settlement cycle closed"
        );
      }
    } catch (err) {
      logger.error({ err }, "IAPAY settlement cycle failed — batch will be retried next interval");
    }
  }, intervalMs);
  timer.unref(); // never keep the process alive just for settlement

  logger.info({ hours }, "IAPAY settlement scheduler started");
}
