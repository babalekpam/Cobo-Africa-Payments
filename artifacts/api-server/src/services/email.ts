...
async function getTransporter() {
  ...
  if (process.env.MAILGUN_SMTP_PASSWORD) {
    transporter = nodemailer.createTransport({
      ...
      port: 587,
      auth: { user: process.env.MAILGUN_SMTP_USER || `postmaster@${process.env.MAILGUN_DOMAIN || "iapay.africa"}`, pass: process.env.MAILGUN_SMTP_PASSWORD },
    });
    return transporter;
  }
...
async function send(to: string, subject: string, body: string) {
  ...
  try {
    await t.sendMail({
      from: `"IAPAY Payments" <${process.env.FROM_EMAIL || "noreply@iapay.africa"}>`,
      to,
      subject,
      html: wrap(body),
    });
...
export const emailService = {
  sendWelcomeEmail: (u: any) =>
    send(u.email, "Welcome to IAPAY", `<h2 style="color:${G}">Welcome, ${u.firstName || u.first_name}!</h2><p style="color:#2C2416;margin-bottom:20px">Your IAPAY account is ready.</p><a href="${BASE}/verification" class="btn">Complete Verification</a>`),

  sendPasswordResetEmail: (u: any, url: string) =>
    send(u.email, "Reset your IAPAY password", `<h2 style="color:${G}">Password Reset</h2><p style="color:#2C2416;margin-bottom:20px">Hi ${u.firstName || u.first_name}, click below to reset your password. <strong>Expires in 1 hour.</strong></p><a href="${url}" class="btn">Reset Password</a><p style="color:#9A8F75;font-size:12px;margin-top:20px">If you didn't request this, ignore this email.</p>`),

  sendPasswordChangedEmail: (u: any) =>
    send(u.email, "Your IAPAY password was changed", `<h2 style="color:${G}">Password Changed</h2><p style="color:#2C2416">Hi ${u.firstName || u.first_name}, your IAPAY password was changed. If this wasn't you, contact support immediately.</p>`),
